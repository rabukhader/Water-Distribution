...
class Formatter {
  ...
  static String convertToYearsMonth(String ageString) {
      ...
      String formattedAge = '';
      if (years > 0) {
        formattedAge += '$years ${years > 1 ? 'years' : 'year'}';
      }
      if (months > 0) {
        if (formattedAge.isNotEmpty) {
          formattedAge += ' and ';
        }
        formattedAge += '$months ${months > 1 ? 'months' : 'month'}';
      }

      return formattedAge.isNotEmpty ? formattedAge : '0 month';
    }
}
